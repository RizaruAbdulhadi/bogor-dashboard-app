...
function MasterBank() {
    ...
    const fetchBanks = async () => {
        try {
            const res = await axios.get('http://localhost:5000/api/rekening');
            setBanks(res.data);
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post('http://localhost:5000/api/rekening', form);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`http://localhost:5000/api/rekening/${id}`);
